...
//
// trmx-80 (FR-13): the VALUE backend is no longer localStorage. All storage-less store instances
// share ONE snapshot — module-level until trmx-253, now owned by a settings RUNTIME —
...
//
// trmx-253 (M8): the ten pieces of module-level state this file used to own — snapshot,
// clientWarnings, configPath, fileWarnings, writeSeq, writeFailedKeys, configWarningsListeners,
// configInvoke, busSubscribed, busUnlistens — live inside `createSettingsRuntime()`, one closure
// per boot. Nothing operational is left at module scope: what remains here is the enumerable
// REGISTRY (keys, defaults, parse/coerce) plus pure helpers, which are values, not state.
// `settingsRuntime.moduleState.test.ts` guards that per piece.
//
// T3.5 deleted what used to sit below the runtime: `makeLegacyStorageStore` (a per-instance
// localStorage backend, test-only since trmx-80 but shipped in production code anyway),
// `__resetSettingsForTest` (a global reset for state that is no longer global), and the compat
// facade of free functions — makeSettingsStore / hydrateSettings / getConfigWarnings / … — that
// resolved them all to one ambient runtime. `createSettingsRuntime()` is now the ONLY way in:
// production builds one in main.tsx and passes it down the React tree; a test builds its own
// through src/test/settingsRuntime.tsx.
...
// ---------------------------------------------------------------------------------------------
// Pure helpers over the registry. NONE of these closes over state — they are safe at module scope
// and shared by every runtime instance and by the legacy storage shim alike.
// ---------------------------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------------------------
// trmx-253 (T3.2, M8): the settings RUNTIME — one explicit object owning every piece of state the
// file-backed settings path needs. Construct one per boot (main.tsx) or one per test; two runtimes
// share nothing at all (settingsRuntime.isolation.test.ts pins that across all ten pieces).
// ---------------------------------------------------------------------------------------------

/** Construction-time dependency injection; `hydrate()` may override any of them per call. */
...
/** The explicit handle production and tests reach the file-backed settings through. */
export interface SettingsRuntime {
  /**
   * The ONE config_read at boot: seed the snapshot, run the legacy-localStorage migration,
   * materialize the first-run theme, subscribe to the bus. Never throws.
   */
  hydrate(deps?: HydrateSettingsDeps): Promise<void>;
  /** Build a store over THIS runtime's snapshot. `bus`/`source` tag the change broadcasts. */
  makeStore(bus?: SettingsBus, source?: string): SettingsStore;
  /** The merged warning ledgers (file-rendered first, then the client-authored per-key ones). */
  getConfigWarnings(): ConfigWarningItem[];
  /** Subscribe to warning changes; the returned function unsubscribes. */
  onConfigWarningsChanged(cb: (items: ConfigWarningItem[]) => void): () => void;
  /** The absolute path of the backend config file, once hydrated (null before / without one). */
   ...
   * rejection PROPAGATES to the caller so the row can surface the failure. Rides this runtime's
  ...
  /**
   * Drop every subscription and every piece of state this runtime holds, returning it to its
   * pre-hydration condition. Deliberately leaves the handle USABLE: a disposed runtime behaves
   * exactly like a freshly created one, which is what a test wants when a single case needs to
   * hydrate twice from different backends. Between CASES nothing needs disposing — a per-test
   * runtime is already isolated, which is why T3.5 could delete the old global reset outright.
   */
  dispose(): void;
}

/**
 * Create a settings runtime. Every piece of state below is a closure local: two runtimes share
 * NOTHING — not the snapshot, not the config path, not either warning ledger, not the write
 * tickets, not the warning subscribers, not the invoke channel, not the bus subscription.
 */
export function createSettingsRuntime(runtimeDeps: SettingsRuntimeDeps = {}): SettingsRuntime {
  // (1) the shared value snapshot for every store this runtime builds.
  const snapshot = new Map<SettingKey, SettingsValues[SettingKey]>();
  // (2) the absolute config-file path, learned at hydration.
  ...
  // (7) the warning subscribers.
  const configWarningsListeners = new Set<(items: ConfigWarningItem[]) => void>();
  // The invoke used for config_write/config_reset_all after (or before) hydration. hydrate() swaps
  // in its injected invoke so every store this runtime builds writes through the same channel.
  // (8) the write channel.
  let configInvoke: InvokeFn = runtimeDeps.invoke ?? realInvoke;
  // (9) whether this runtime has already subscribed to the bus.
  ...
  function getConfigWarnings(): ConfigWarningItem[] {
   ...
   */
  function onConfigWarningsChanged(
  ...
  /** trmx-80: the production store over THIS runtime's shared snapshot. */
  function makeSnapshotStore(bus?: SettingsBus, source: string = "unknown"): SettingsStore {
   ...
   */
  async function hydrate(deps: HydrateSettingsDeps = {}): Promise<void> {
    configInvoke = deps.invoke ?? runtimeDeps.invoke ?? realInvoke;
    const bus = deps.bus ?? runtimeDeps.bus ?? realEventBus;
    const storage = deps.storage ?? runtimeDeps.storage ?? safeLocalStorage();
  ...
  return {
    hydrate,
    makeStore: makeSnapshotStore,
    getConfigWarnings,
    onConfigWarningsChanged,
    getConfigFilePath: () => configPath,
    openConfigFile: () => invokeSafely("config_open_file").then(() => {}),
    getLogDir: () => invokeSafely("log_dir").then((dir) => String(dir)),
    openLogDir: () => invokeSafely("log_open_dir").then(() => {}),
    dispose() {
      snapshot.clear();
      configPath = null;
      fileWarnings = [];
