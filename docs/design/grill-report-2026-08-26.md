...
2. **Live panes (100 tabs × splits).** Two costs compound: (a) `App.tsx` fans every reducer dispatch through 20 `useEffect`s and 15 ref-maps (H6); (b) the 1 Hz foreground poller resolves each session via `Command::new("ps")` (`platform/src/foreground.rs:407-417`, driven from `main.rs:66-410`) — *if* it does one fork per session per tick (not verified — see Assumptions audit A3), 100 panes ≈ 100–200 `ps` forks/second on the main thread's behalf. Validate before optimizing; if confirmed, batch into one `ps -o ... -p pid1,pid2,...` per tick.
...
| A3 | The 1 Hz poller forks `ps` once per session per tick, making it the second scale cliff (Scale stress §2) | this synthesis (inferred from `foreground.rs:407-417` + `main.rs:66-410`; **not verified**) | Open 50 panes; `sudo fs_usage -w -f exec \| grep -c ps` for 10 s; or read `run_title_poller` | 30 min |
...
| Scale cliff #2 (`ps` per session) | **Low** — inferred, not verified (A3) | 30-minute `fs_usage` measurement |
