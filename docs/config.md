...
| `shell.prompt` | `shell.prompt` | string | `"existing"` | `existing` (your own prompt, untouched) · `starship` (bundled sidecar; honors your `starship.toml`) · `powerlevel10k` (bundled, wizard suppressed, no-network gitstatus fallback; honors `~/.p10k.zsh`) · `pure` (bundled) — trmx-207 | new sessions only |
